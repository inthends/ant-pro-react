 ...
 * @type R: is route
 */
export interface MenuDataItem<R extends boolean = true> {
...
export interface BaseMenuProps extends ConnectProps {
  className?: string;
  collapsed?: boolean;
  flatMenuKeys?: any[];
...
class BaseMenu extends Component<BaseMenuProps> {
  ...
  getSecondMenu = ({path}) => {    
    const { isMobile, onCollapse,dispatch } = this.props;
    isMobile ? () => onCollapse!(true) : void 0;
    dispatch!({
      type: 'menu/setSecondMenu',
      payload: { path },
    });
  }
  /**
  ...
  getMenuItemPath = (item: MenuDataItem) => {
    ...
    return (
      <Link
        ...
        onClick={() => this.getSecondMenu(item)}
      >
        {icon}
        <span>{name}</span>
      </Link>
