...
export default class Ellipsis extends Component {
  ...
  componentDidUpdate(perProps) {
    ...
    if (lines !== perProps.lines || maxLines !== perProps.maxLines) {
      this.computeLine();
    }
  }
  ...
  computeLine = () => {
    const { lines, maxLines } = this.props;
    const newLines = maxLines || lines;

    if (newLines && !isSupportLineClamp) {
      const text = this.shadowChildren.innerText || this.shadowChildren.textContent;
      const lineHeight = parseInt(getComputedStyle(this.root).lineHeight, 10);
      const targetHeight = newLines * lineHeight;
      if (maxLines) {
