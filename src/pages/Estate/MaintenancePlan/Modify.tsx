...
import {MovePoint, SaveFormPlan, GetLinePonitPageListById, RemoveLinePoint, RemoveLinePointAll } from "./Main.service";
...
const Modify = (props: ModifyProps) => {
  ...
  const doSave = dataDetail => {
    ...
    return SaveFormPlan(modifyData);
  };
  ...
  //初始化
  const initLoadData = (search, planId) => {
    const queryJson = {
      keyword: search,
      PlanId: planId
    }
  ...
  const [planDeviceId, setPlanDeviceId] = useState<any>();
  const doModify = record => {
    setPlanDeviceId(record.id);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        return [
          <span key='span'>
            <a onClick={() => doModify(record)} key="modify">维保内容</a>
            <Divider type="vertical" key='divider2' />
            <a onClick={() => { 
              MovePoint(record.id,0).then(res => {
                initLoadData(search, planId);
              })
            }
            } key="modify">上移</a>
